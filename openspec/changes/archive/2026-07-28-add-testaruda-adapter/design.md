...
- **Status: RESOLVED.** Confirmed against testaruda's actual `AdapterIO` implementation (`src/adapter.rs`, v0.2.3 commit `34e8db6`): `AdapterIO::spawn` performs the handshake once and returns a handle holding the `Child`; all subsequent commands (`discover`/`static_deps`/`fingerprint`/`run_args`/`ingest`) reuse the same process via `send()`; `kill()` consumes `self` at end. `main.rs` spawns one adapter per testaruda invocation and issues multiple commands against it. The long-lived model is exactly how `AdapterIO` works; per-command spawning is not how the current core operates.
...
- **Status: RESOLVED (for the current testaruda core).** Confirmed against testaruda v0.2.3 (`34e8db6`): (1) `store.rs` hardcodes `component = "default"` for all adapter data (`store_discovered_tests`, `store_static_deps`) — TIA-COMP-008 (parallel per-component selection) is specified but not implemented in the store; (2) `engine.rs` runs one flat Ascent program over all content-units/tests with `test_comp` used only for fallback scoping, not for per-component adapter queries; (3) the Julia adapter change (`add-julia-adapter-via-testimonial` Decision 5) poses the identical multi-package scoping question and notes whole-repo discover "composes with testaruda's existing single-store architecture."
- **Shared-open-question caveat**: The whole-repo-vs-per-component question is not titi-specific. If testaruda later implements TIA-COMP-008 (parallel per-component selection), adapters that span multiple logical components may need re-scoping. This is a testaruda-side spec clarification tracked by the Julia adapter change (Decision 5); titi proceeds with option (b) against the current core and will revisit if testaruda's composability direction changes. The `spec implication` noted below (TIA-COMP-003 intentionally not exercised for adapter-owned components) is therefore accurate for the current core.
- **Spec implication**: `composability`'s TIA-COMP-003 is intentionally not exercised for adapter-owned components — a spec clarification testaruda's maintainers need to make.
