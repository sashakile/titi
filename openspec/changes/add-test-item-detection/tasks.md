...
- [x] 2.6 Cache test-item lists in `.titi/test-cache/items/`  (→ beads titi-tdq), invalidated when `.csproj` or test source files change
...
- [x] 3.6 Incremental edge update: only re-run tests whose source-fingerprint changed  (→ beads titi-8sg)
...
- [x] 5.4 Upgrade `titi test-manifest` with `--select` flag  (→ beads titi-nxo): when enabled, emit per-test-filtered Traversal using `dotnet test --filter`; framework-aware filter syntax (xUnit `~`, NUnit `==`, MSTest `TestCategory` or `~`); batch splitting when filter >4000 chars
- [x] 5.5 Upgrade `titi test-manifest` with `--list` flag  (→ beads titi-nxo): print selected test IDs instead of emitting a Traversal file
...
- [x] 7.4 Verify `titi test-manifest --select`  (→ beads titi-nxo) emits filtered vs unfiltered Traversal correctly; verify framework-aware filter syntax and batch splitting
- [x] 7.5 Verify always-run set  (→ beads titi-nxo) includes failed/new/no-history tests; verify "newly added" relative to recording (not discovery)
- [x] 7.6 Verify confidence fallback  (→ beads titi-nxo) triggers when below threshold
...
## 8. Documentation
- [x] 8.1 Document test-detection capability in README  (→ beads titi-yhm)
- [x] 8.2 Document `titi tests list`, `titi tests ingest`, `titi tests record`  (→ beads titi-yhm) in CLI reference
- [x] 8.3 Document safety invariants and confidence model  (→ beads titi-yhm)
- [x] 8.4 Document relation to `add-testaruda-adapter` Phase 2  (→ beads titi-yhm) (method-level granularity builds on these primitives; this change is a prerequisite, not a replacement)
