...
| `cache-dir` | string | `".titi/test-cache"` | Root directory for all test-related caches: edges (`<cache-dir>/edges/`), items (`<cache-dir>/items/`), and run history (`<cache-dir>/history.jls`) |
...
>   history.jls                 # run history persistence (TD-06)
> ```
